...
class FedAvg(FlowerFedAvg):
    ...
    def __init__(self, ckp, client_valuation, *args, log=False, aggregate_equal_weights=False, **kwargs):
        ...
        self.validation_set = hasattr(
            data_config.args, 'val_ratio') and data_config.args.val_ratio > 0.
    ...
    def aggregate_fit(
        ...
        for k, v in train_summary.items():
            ...
            self.ckp.log({f'var_{k}': np.mean(v)},
                         step=server_round, commit=False)
        ...
        if self.log:
            ...
            client_weights = [parameters_to_ndarrays(
                fit_res.parameters) for _, fit_res in results]
            client_samples = [fit_res.num_examples for _, fit_res in results]

            self.log_round(server_round, client_cids, client_samples,
                           client_weights, aggregated_weights, current_weights)

        return ndarrays_to_parameters(aggregated_weights), {}

    def log_round(self, server_round: int, client_cids: List[int], client_samples: List[int], client_weights: List[NDArrays], aggregated_weights: NDArrays, current_weights: NDArrays):
        round_config = self.on_fit_config_fn(server_round)
                ...
                if type(v[0]) != float:
                    ...
                    for class_id, class_v in enumerate(list(zip(*v))):
                        if len(class_v) > 1:
                            ...
                            tmp[f'{log_name} | Class{class_id}'] = np.mean(
                                class_v)
                        else:
                            log_name = f'{name} | {k}'
                    ...
                    self.ckp.log(tmp, step=server_round, commit=False)
                else:
                    # log mean across clients for each metric
                    if len(v) > 1:
                        self.ckp.log({f'{name} | mean_{k}': np.mean(
                            v)}, step=server_round, commit=False)
                    else:
                        self.ckp.log(
                            {f'{name} | {k}': v[0]}, step=server_round, commit=False)
    ...
    def evaluate(
        ...
        eval_res = self.evaluate_fn(weights, partition)
        if eval_res is None:
            return None
        loss, other = eval_res
...
def weighted_loss_avg(results: Dict[str, Tuple[int, float, Optional[Dict[str, float]]]], personalized_fl_groups: Dict[str, int]) -> Tuple[float, float]:
    ...
    if personalized_fl_groups is not None and len(personalized_fl_groups) > 1:
        ...
        for group, to_id in personalized_fl_groups.items():
            ...
            for k, v in group_correct_preds.items():
                if 'test_acc' in k or 'accuracy' in k:
                    accuracy_results[f'ps_{k}_alpha{group}({to_id} clients)'] = v / \
                        group_examples * 100
                else:
                    accuracy_results[f'mean_{k}_alpha{group}({to_id} clients)'] = v / float(
                        to_id)
    ...
    weighted_losses = [num_examples *
                       loss for num_examples, loss, _ in results.values()]
    ...
    for k, v in num_correct_preds.items():
        if 'test_acc' in k or 'accuracy' in k:
            accuracy_results[f'ps_{k}'] = sum(
                v) / num_total_evaluation_examples * 100
        else:
