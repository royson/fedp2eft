...
class Checkpoint():
    ...
    def init_wandb(self):
        ...
        if 'mode' in wandb_args and wandb_args['mode'] == 'disabled':
            # wandb is disabled. create dir name given run name
            wandb_dir = './wandb'
            wandb_dir = os.path.abspath(os.path.join(wandb_dir, self.name, self.args['run_id']))
            os.makedirs(wandb_dir)
            wandb.run.dir = wandb_dir
        else:
            # wandb is enabled. create sym link w run name to wandb UID dir
            os.symlink(Path(wandb.run.dir).parent.absolute(), 
                       Path(wandb.run.dir.replace(wandb.run.id, wandb.run.name)).parent.absolute(), 
    ...
    def offline_save(self, filepath, value, use_wandb_dir=False):
        '''
        Save to local filepath only. Does not upload to wandb until a sync is initialized.
        '''

        if use_wandb_dir:
            filepath = os.path.join(self.run_dir, filepath)
        # create parent directories if they don't exist
        if os.path.dirname(filepath): 
            os.makedirs(os.path.dirname(filepath), exist_ok=True)

        with open(os.path.join(filepath), 'wb') as handle:
            pickle.dump(value, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def offline_load(self, filepath, use_wandb_dir=False):
        '''
    ...
    def load_model_from_run_id(self, wandb_run_path, 
        ...
        print("pretrain_local_path ", pretrain_local_path, " exists ", os.path.exists(pretrain_local_path))
        ...
        if not os.path.exists(pretrain_local_path):
            ...
            try:
                run_api = api.run(wandb_run)
                print("wandb run ", wandb_run,"state",  run_api.state)
            except:
                logger.error(f'{wandb_run} not found')

            if run_api.state == 'crashed':
                logger.error(f'{wandb_run} crashed')
                            
            run_api.file(wandb_folder).download(replace=False, root=pretrain_path)
    ...
    def load_pretrain_net_parameters(self, local_path_to_model):
        '''
        Loading of pretrained model
        '''
        wandb_args = self.args['wandb_args']

        if os.path.exists(local_path_to_model):
            # always corrupted file
            pretrain_parameters = torch.load(local_path_to_model)
            
        else:
            pretrain_parameters = self.load_model_from_run_id(local_path_to_model)
            pretrain_parameters = parameters_to_ndarrays(pretrain_parameters)
            
        if 'mode' in wandb_args and wandb_args['mode'] != 'disabled':
            wandb.save(local_path_to_model)
