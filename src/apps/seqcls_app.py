...
class SeqClsApp(App):    
    def __init__(self, *args, test_only=False, skip_initial_eval=True, save_final_model=True, **kwargs):
        ...
        if 'load_local_layers' in self.config.app.client.args and self.config.app.client.args.load_local_layers:
            net = arch_fn(0, **net_config.args, tokenizer=tokenizer)
        else:
