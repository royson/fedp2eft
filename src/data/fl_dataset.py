...
from src.data.common import shuffle, sort_by_label, split_array_at_indices, sample_without_replacement
...
def create_lda_partitions(
    ...
    if quantity_skew_dirichlet is None:
        ...
        num_samples = np.random.multinomial(
            x.shape[0] - (2 * num_partitions), quantity_skew_dirichlet)
        num_samples += 2
...
class FederatedDataset(ABC):
    def __init__(self, ckp, path_to_data, dataset_fl_root, *args,
        ...
        if not self.pre_partition:
            ...
            self.lda_alpha = {str(key): value for key,
                              value in self.lda_alpha.items()}
            ...
            self.test_alpha = dict(
                sorted(self.test_alpha.items(), reverse=True))

            for a in self.train_alpha:
                assert str(float(a)) in self.lda_alpha or str(int(
                    a)) in self.lda_alpha, f'Train alpha ({a}) must be found in lda_alpha ({self.lda_alpha})'
    ...
    def create_fl_partitions(self):
        ...
        logger.info(
            f'Lda alpha:{self.lda_alpha}. Test alpha: {self.test_alpha}. Training with {len(self.get_available_training_clients())} clients.')
        ...
        if not self.pre_partition:
            ...
            for alpha, num_clients in self.lda_alpha.items():
                if alpha in self.train_alpha:
                    available_clients += list(range(start,
                                              start + num_clients))
                start += num_clients
    ...
    def _create_fl_partition(self, alpha_dict):
        ...
        if self.reset and os.path.exists(dir_path):
            logger.info(
                f'Reset flag is set for data federated splitting.. Deleting current {dir_path}')
        ...
        for partition in self.partitions:
            ...
            server_data_dir = os.path.join(
                self.dataset_fl_root, f'{partition}')
