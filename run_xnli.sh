...
SERVER=server
PRETRAIN_METHOD=fedavg
GPUS=0,1,2,3,4

: '
Base Model: Further training mBERT with Standard FL
...
MBERT_MODEL_PATH=${HOME_DIR}/models/xnli_${PRETRAIN_METHOD}_mbert/model.pt
...
do
    ...
    do
        ...
        wait
        CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_lora.yaml configs/xnli/${SERVER}.yaml $ADDITIONAL_COMMANDS wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_lora_unseen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_unseen_r${rank}_rp${RUNS[i]} data.args.pool=unseen  &
        wait
    done
done
...
for i in "${!RUNS[@]}"
do
    ADDITIONAL_COMMANDS='configs/xnli/app.on_evaluate.lr=0.001 seed='${SEED[i]}' models.net.args.seed='${SEED[i]}' models.net.args.model_name_or_path='$MBERT_MODEL_PATH''
    for j in "${!RANKS[@]}"
    do
        ...
        wait
        CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_adalora.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_adalora_unseen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_adalora_unseen_r${RANKS[j]}_rmul1.5_rp${RUNS[i]} data.args.pool=unseen models.net.args.adapter_args.target_r=${RANKS[j]} models.net.args.adapter_args.init_r=${INIT_RANKS[j]} $ADDITIONAL_COMMANDS &
        wait
    done
done
...
do
    ...
    do
        if [[ ${RANKS[j]} -eq ${RANKS[@]:0:1} ]]; then
            ...
            wait
            CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_btlora.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_bayestune_unseen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_bayestune_unseen_r${RANKS[j]}_rp${RUNS[i]} models.net.args.adapter_args.r=${BT_MAX_RANK} models.net.args.adapter_args.lora_alpha=$((${BT_MAX_RANK} * 2)) data.args.pool=unseen $ADDITIONAL_COMMANDS &
            wait
        else
            echo NEXT${RANKS[j]}
            ...
            CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_btlora.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_bayestune_seen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_bayestune_seen_r${RANKS[j]}_rp${RUNS[i]} models.net.args.adapter_args.r=${BT_MAX_RANK} models.net.args.adapter_args.lora_alpha=$((${BT_MAX_RANK} * 2)) $ADDITIONAL_COMMANDS &
            wait
            BL_BTS=${HOME_DIR}/models/xnli_${PRETRAIN_METHOD}_mbert_finetune_bayestune_unseen_r${RANKS[@]:0:1}_rp${RUNS[i]}
            ADDITIONAL_COMMANDS='seed='${SEED[i]}' models.net.args.seed='${SEED[i]}' app.client.args.bt_args.eval_rank='${RANKS[j]}' models.net.args.model_name_or_path='$MBERT_MODEL_PATH' app.client.args.bt_args.load_bts_path='$BL_BTS''
            CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_btlora.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_bayestune_unseen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_bayestune_unseen_r${RANKS[j]}_rp${RUNS[i]} models.net.args.adapter_args.r=${BT_MAX_RANK} models.net.args.adapter_args.lora_alpha=$((${BT_MAX_RANK} * 2)) data.args.pool=unseen $ADDITIONAL_COMMANDS &
            wait
        fi 
...
do
    ...
    do
        ...
        wait
        CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_fedl2p.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_fedl2p_unseen name=xnli_${PRETRAIN_METHOD}_mbert_finetune_fedl2p_unseen_r${rank}_rp${RUNS[i]} app.client.args.eval_rank=${rank} models.net.args.adapter_args.r=${rank} models.net.args.adapter_args.lora_alpha=$((rank * 2)) app.args.load_fedl2p_params=$MBERT_FEDL2P app.args.test_only=True data.args.pool=unseen $ADDITIONAL_COMMANDS &
        wait
    done
done
...
for i in "${!RUNS[@]}"
do
    ADDITIONAL_COMMANDS='app.client.args.bts_lr='${BTS_LR}' app.client.args.task_lr=0.0001 app.client.args.loss_weights.sparsity='${SPARSITY_WEIGHT}' app.client.args.loss_weights.significance='${SIG_WEIGHT}' seed='${SEED[i]}' models.net.args.seed='${SEED[i]}' models.net.args.model_name_or_path='$MBERT_MODEL_PATH' models.net.args.adapter_args.r='${BT_MAX_RANK}' models.net.args.adapter_args.lora_alpha='$((${BT_MAX_RANK} * 2))' app.run.num_rounds=150 app.run.test_every_n=150 app.run.save_every_n=150'
    ...
    do
        ...
        do
            ...
            wait
            CUDA_VISIBLE_DEVICES=${GPUS} python main.py configs/xnli/xnli_fedp2eft.yaml configs/xnli/${SERVER}.yaml wandb_args.group=xnli_${PRETRAIN_METHOD}_mbert_finetuning_fedp2eft_unseen_btslr${BTS_LR}_sp${SPARSITY_WEIGHT}_sig${SIG_WEIGHT}_mr${BT_MAX_RANK}_rnd${round} name=xnli_${PRETRAIN_METHOD}_mbert_finetune_fedp2eft_unseen_btslr${BTS_LR}_sp${SPARSITY_WEIGHT}_sig${SIG_WEIGHT}_mr${BT_MAX_RANK}_rnd${round}_r${rank}_rp${RUNS[i]} app.client.args.eval_rank=${rank} app.args.load_fedl2p_params=$MBERT_FEDP_RW app.args.test_only=True data.args.pool=unseen $ADDITIONAL_COMMANDS &
            wait
        done
    done
done